...
// Load Playwright Test Execution Results
let latestRunInfo = '100% Passing (77 Scenarios)';
let latestRunBadge = 'All Suites Verified';
...
if (fs.existsSync(resultsJsonPath)) {
  try {
    ...
    function scanSuites(suite) {
        ...
        for (const spec of suite.specs) {
          if (spec.tests) {
            for (const t of spec.tests) {
              totalTests++;
              const hasFailure = t.results && t.results.some(r => r.status === 'failed' || r.status === 'timedOut');
      ...
      if (suite.suites) {
        suite.suites.forEach(scanSuites);
      }
    }

    if (testResults.suites) {
      testResults.suites.forEach(scanSuites);
    }
    ...
    if (totalTests > 0) {
      ...
      if (failedTests === 0) {
        ...
        latestRunInfo = `${suiteLabel} (${failedTests} Failed, ${passedTests} Passed)`;
        latestRunBadge = `${suiteLabel}: Issues Detected`;
      }
    }
  } catch (e) {
...
<body>
  ...
  <div class="container">
    ...
    <div id="view-post" class="view-content">
      <div class="panel">
        ...
        <div class="grid-4" style="margin-bottom: 1.5rem;">
          <div class="stat-card green">
            <div class="label">Composer Launch</div>
            <div class="value" style="font-size: 1.4rem;">New Post</div>
            <div class="subtext"><span>✅</span> TC_POST_001 Verified</div>
          </div>
          <div class="stat-card blue">
            <div class="label">Target Form Selector</div>
            <div class="value" style="font-size: 1.4rem;">@m_2094 / posts</div>
            <div class="subtext"><span>✅</span> TC_POST_002 Verified</div>
          </div>
          <div class="stat-card purple">
        ...
        <table>
          ...
          <tbody>
            <tr>
              <td><code>TC_POST_001</code></td>
              <td>Home page "New Post" button & modal launch</td>
              <td><code>button:has-text("New Post")</code></td>
              <td>Opens "CREATE FORMS" target selector modal</td>
              <td><span class="badge passed">PASSED</span></td>
            </tr>
            <tr>
              <td><code>TC_POST_002</code></td>
              <td>Target form selection & composer view transition</td>
              <td><code>div:has-text("posts")</code></td>
              <td>Opens composer with "NEW POST IN" header</td>
              <td><span class="badge passed">PASSED</span></td>
            </tr>
            <tr>
              <td><code>TC_POST_003</code></td>
              <td>Composition tools & actions verification</td>
              <td><code>Add Text, Media, Longform, Link, Quote, Mention</code></td>
              <td>All tool buttons active and accessible</td>
              <td><span class="badge passed">PASSED</span></td>
            </tr>
            <tr>
              <td><code>TC_POST_004</code></td>
      ...
      <div class="panel">
        <div class="panel-header">
          <div>
            ...
            <p style="font-size: 0.85rem; color: var(--text-muted); margin-top: 0.25rem;">
              Workflows: <code>.github/workflows/*.yml</code> | Timeout: <strong>90 Minutes (1h 30m)</strong> | Supports Manual UI & Automated Triggers
            </p>
          </div>
          <a href="https://github.com/mughdabansal/Vakh-Playwright--test-/actions" target="_blank" class="btn-github">
            View on GitHub &rarr;
        ...
        <div class="grid-4" style="margin-bottom: 1.5rem;">
          <div class="stat-card green">
            <div class="label">Trigger Conditions</div>
            <div class="value" style="font-size: 1.15rem;">Manual UI & Push</div>
            <div class="subtext">workflow_dispatch & push</div>
          </div>
          <div class="stat-card blue">
            <div class="label">Runner Timeout</div>
            <div class="value" style="font-size: 1.25rem;">90 Minutes</div>
            <div class="subtext">Increased to 1h 30m</div>
          </div>
          <div class="stat-card purple">
            <div class="label">Modular Page Suites</div>
            <div class="value" style="font-size: 1.25rem;">6 Pages</div>
            <div class="subtext">Login, Home, Chat, Activity, Explore, Post</div>
          </div>
          <div class="stat-card orange">
        ...
        <h4 style="margin-bottom: 0.75rem; color: #cbd5e1;">🚀 Page-Specific CI/CD Execution Matrix</h4>
        <table>
          <thead>
            <tr>
              <th>Module / Page</th>
              <th>Single Command Package</th>
              <th>Dedicated GitHub Actions Workflow</th>
          ...
          <tbody>
            <tr>
              ...
              <td><code>Page Suite: Login Page (.github/workflows/test-login.yml)</code></td>
              <td>Auto-generates & deploys on completion</td>
            </tr>
            <tr>
              <td>🏠 <strong>Home Page</strong></td>
              <td><code>npm run test:home</code></td>
              <td><code>Page Suite: Home Page (.github/workflows/test-home.yml)</code></td>
              <td>Auto-generates & deploys on completion</td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td><code>npm run test:activity</code></td>
              <td><code>Page Suite: Activity Page (.github/workflows/test-activity.yml)</code></td>
              <td>Auto-generates & deploys on completion</td>
            </tr>
            <tr>
              <td>🧭 <strong>Explore Page</strong></td>
              <td><code>npm run test:explore</code></td>
              <td><code>Page Suite: Explore Page (.github/workflows/test-explore.yml)</code></td>
              <td>Auto-generates & deploys on completion</td>
            </tr>
            <tr>
              <td>📝 <strong>Create Post Page</strong></td>
        ...
        <h4 style="margin-top: 1.5rem; margin-bottom: 0.75rem; color: #cbd5e1;">🚀 Pipeline Execution Flow</h4>
        <div class="timeline">
          ...
          <div class="timeline-step">
            ...
            <div class="step-info">
              ...
              <div class="step-desc"><code>actions/setup-node@v4</code> &mdash; Sets up Node 22 environment with npm caching.</div>
            </div>
            <div class="step-badge">SUCCESS</div>
          </div>
          ...
          <div class="timeline-step">
            ...
            <div class="step-info">
              <div class="step-title">Page-Specific or Full Suite Test Execution</div>
              <div class="step-desc">Runs chosen page command (e.g. <code>npm run test:login</code>) with 90-minute timeout guard.</div>
            </div>
            <div class="step-badge">PASSED</div>
          </div>
          ...
          <div class="timeline-step">
            ...
            <div class="step-badge">UPDATED</div>
          </div>
    ...
    <footer>
      ...
      <p style="margin-top: 0.35rem; color: var(--text-dim);">Live Dashboard: <a href="https://mughdabansal.github.io/Vakh-Playwright--test-/" style="color: #60a5fa;">https://mughdabansal.github.io/Vakh-Playwright--test-/</a> | Last updated: <strong>${lastUpdated}</strong></p>
    </footer>
